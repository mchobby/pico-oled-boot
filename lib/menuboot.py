""" MicroPython Menu for PICO-OLED-BOOT - use I2C OLED + I2C MCP23008
...
class RangeControler:
	...
	def start( self ):
		...
		while self.owner.oled.dir==ENTER:
			time.sleep_ms( 10 )
	...
	def draw( self, oled ):
		...
		oled.fill_rect( 0, 20, wfill, 12, 1 ) # fill
	...
	def update( self ):
		...
		_dir = oled.dir
		if (_dir==RIGHT) and (time.ticks_diff(time.ticks_ms(),self._last_change)>DIR_REPEAT_MS):
			self.inc()
			self._last_change=time.ticks_ms()
		elif (_dir==LEFT) and (time.ticks_diff(time.ticks_ms(),self._last_change)>DIR_REPEAT_MS):
			self.dec()
			self._last_change=time.ticks_ms()
...
class ComboControler:
	...
	def start( self ):
		...
		self._submenu = MenuBoot( self.owner.oled )
		_focus_idx = None
		_idx = 0
...
class ScreenControler:
	...
	def update( self ):
		...
		if self._on_draw != None:
			self._on_draw( self, self.owner.oled )

		# If button is pressed Then we do quit the controler
		if self.owner.oled.dir==ENTER:
			while self.owner.oled.dir==ENTER: # wait the button to be released
				time.sleep_ms(10)
			self.selected = True
...
class MenuBoot:
	def __init__( self, oled_boot ):
		...
		self._last_dir = 0     # Last direction selected on joystick
		self._last_dir_time = time.ticks_ms()
	...
	def update( self ):
		...
		# What to focus
		if (_dir>0) and ((_dir!=self._last_dir)or(time.ticks_diff(time.ticks_ms(), self._last_dir_time)>DIR_REPEAT_MS)):
			if _dir==DOWN > 0:
				self.focus_next()
			elif _dir==UP:
				self.focus_prev()
		...
		if (_dir==ENTER) and self._last_dir:
